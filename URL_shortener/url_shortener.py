...
class url_shortener:
        ...
        def __init__(self):
                ...
                Label(self.root,text="Paste Your Url Here ..",font=('verdana',10,'bold'),fg="blue").place(x=50,y=50)

                self.url = Entry(self.root,width=50,bg="lightgrey",relief=GROOVE,borderwidth=2,border=2)
                self.url.place(x=50,y=80)

                self.button = Button(self.root,relief=GROOVE,text="Shorten",font=('verdana',8,'bold'),bg="purple",fg="white",command=self.create)
